...
@pytest.mark.asyncio
async def test_concurrent_identical_requests_run_once(monkeypatch):
    calls = []
...
@pytest.mark.asyncio
async def test_each_caller_gets_its_own_object(monkeypatch):
...
@pytest.mark.asyncio
async def test_sequential_requests_are_not_shared(monkeypatch):
...
@pytest.mark.asyncio
async def test_fresh_opts_out(monkeypatch):
    calls = []
    monkeypatch.setattr(fetch, "_execute", _counting_execute(calls))
...
@pytest.mark.asyncio
async def test_posts_are_never_coalesced(monkeypatch):
...
@pytest.mark.asyncio
async def test_different_identities_are_not_shared(monkeypatch):
...
@pytest.mark.asyncio
async def test_logged_out_request_is_not_shared_with_logged_in(monkeypatch):
    calls = []
    monkeypatch.setattr(fetch, "_execute", _counting_execute(calls))
...
@pytest.mark.asyncio
async def test_failure_propagates_and_clears(monkeypatch):
